...
class MechanicsVisualizer:
    ...
    def save_animation_to_file(
        self, anim: animation.FuncAnimation, filename: str, fps: int = None, dpi: int = 100
    ) -> bool:
        """
    ...
    def _animate_single_pendulum(self, t: np.ndarray, y: np.ndarray, parameters: dict):
        ...
        (line,) = self.ax.plot(
            [], [], [], "o-", linewidth=3, markersize=10, color=PRIMARY_COLOR, label="Pendulum"
        )
        (trail,) = self.ax.plot(
            [], [], [], "-", alpha=TRAIL_ALPHA, linewidth=1.5, color=SECONDARY_COLOR, label="Trail"
        )
        ...
        self.animation = animation.FuncAnimation(
            self.fig, animate_frame, frames=len(t), interval=interval, blit=False, repeat=True
        )
    ...
    def _animate_double_pendulum(self, t: np.ndarray, y: np.ndarray, parameters: dict):
        ...
        (line1,) = self.ax.plot(
            [], [], [], "o-", linewidth=3, markersize=10, color=PRIMARY_COLOR, label="Pendulum 1"
        )
        (line2,) = self.ax.plot(
            [], [], [], "o-", linewidth=3, markersize=10, color=TERTIARY_COLOR, label="Pendulum 2"
        )
        (trail1,) = self.ax.plot([], [], [], "-", alpha=0.3, linewidth=1, color=PRIMARY_COLOR)
        (trail2,) = self.ax.plot(
            [], [], [], "-", alpha=TRAIL_ALPHA, linewidth=1.5, color=SECONDARY_COLOR
        )
        time_text = self.ax.text2D(0.05, 0.95, "", transform=self.ax.transAxes, fontsize=12)
    ...
    def plot_energy(
        self, solution: dict, parameters: dict, system_name: str = "", lagrangian: sp.Expr = None
    ):
        ...
        E_error = (
            (E_total - E_total[0]) / np.abs(E_total[0]) * 100
            if E_total[0] != 0
            else (E_total - E_total[0])
        )
