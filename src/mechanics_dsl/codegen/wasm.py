...
"""
    ...
    def _generate_c_source(self) -> str:
        ...
        params = "\n".join(
            f"static const double {name} = {val};" for name, val in self.parameters.items()
        )
 ...
 *   emcc {self.system_name}.c -o {self.system_name}.js -s EXPORTED_FUNCTIONS="['_init','_step','_get_state']" -s EXPORTED_RUNTIME_METHODS="['ccall','cwrap']"  # noqa: E501
 */
    ...
    <script>
        ...
        function draw() {{
            ...
            document.getElementById('state').textContent =
                'θ=' + theta.toFixed(4) + ', θ̇=' + get_state_fn(1).toFixed(4);
        }}
