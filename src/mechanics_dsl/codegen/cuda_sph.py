...
"""
    ...
    def _generate_particle_init(self) -> str:
        ...
        for i, p in enumerate(self.fluid_particles):
            lines.append(
                f"h_x[{i}] = {p.get('x', 0.0)}f; h_y[{i}] = {p.get('y', 0.0)}f; h_type[{i}] = 0;"
            )
...
"""
    ...
    def _generate_cpu_fallback(self) -> str:
        """Generate complete CPU fallback implementation."""
        num_fluid = len(self.fluid_particles)
        num_boundary = len(self.boundary_particles)
        num_total = num_fluid + num_boundary
...
namespace params {{
    ...
    constexpr float VISC_LAP = 45.0f / (PI * std::pow(h, 6));
}}
...
"""
    ...
    def _generate_cpu_particle_init(self) -> str:
        """Generate CPU particle initialization code."""
        lines = []
        ...
        if not lines:
            # Default dam break scenario if no particles provided
            lines.append("    // Default dam break initialization")
            lines.append("    int idx = 0;")
            lines.append("    for (float y = 0.1f; y < 0.5f; y += 0.02f) {")
