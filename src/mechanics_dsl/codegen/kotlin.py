...
class KotlinGenerator(CodeGenerator):
    ...
    def _generate_kotlin_code(self) -> str:
        ...
        # Pre-compute all components to avoid f-string backslash issues in Python < 3.12
        coroutines_import = "import kotlinx.coroutines.*" if self.use_coroutines else ""
        coroutines_flow_import = "import kotlinx.coroutines.flow.*" if self.use_coroutines else ""
        compose_import = "import androidx.compose.runtime.*" if self.use_compose else ""
        coords_list = ', '.join(self.coordinates)
        state_class = self._generate_state_class()
        array_fields = ', '.join([f'{c}, {c}Dot' for c in self.coordinates])
        state_from_array = self._generate_state_from_array()
        parameters = self._generate_parameters()
        state_decl = "    var state by mutableStateOf(initialState())" if self.use_compose else "    var state = initialState()"
        time_decl = "    var time by mutableStateOf(0.0)" if self.use_compose else "    var time = 0.0"
        running_decl = "    var isRunning by mutableStateOf(false)" if self.use_compose else "    var isRunning = false"
        ...
        derivative_list = self._generate_derivative_list()
        suspend_kw = "suspend " if self.use_coroutines else ""
        running_true = "isRunning = true" if self.use_compose else ""
        yield_call = "yield()" if self.use_coroutines else ""
        running_false = "isRunning = false" if self.use_compose else ""
        energy_method = self._generate_energy_method() if self.lagrangian else ""
        
        template = f'''package {self.package_name}

import kotlin.math.*
{coroutines_import}
{coroutines_flow_import}
{compose_import}
...
{equations}
        ...
        return doubleArrayOf({derivative_list})
    }}
     ...
     */
    {suspend_kw}fun run(duration: Double): List<Pair<Double, SimulationState>> {{
