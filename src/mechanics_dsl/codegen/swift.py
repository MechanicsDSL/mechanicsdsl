...
class SwiftGenerator(CodeGenerator):
    ...
    def _generate_swift_code(self) -> str:
        ...
        accelerate_import = "import Accelerate" if self.use_accelerate else ""
        combine_import = "import Combine" if self.use_combine else ""
        state_struct = self._generate_state_struct()
        array_fields = ', '.join([f'{c}, {c}_dot' for c in self.coordinates])
        state_from_array_init = self._generate_state_from_array_init()
        init_params = ', '.join([f'{c}: Double = 0, {c}_dot: Double = 0' for c in self.coordinates])
        state_member_assigns = self._generate_state_member_assigns()
        class_attr = self._get_class_attribute()
        protocol = self._get_protocol_conformance()
        protocol_decl = f': {protocol}' if protocol else ''
        parameters = self._generate_parameters()
        published = self._get_published_prefix()
        ...
        derivative_packing = self._generate_derivative_packing()
...
{state_from_array_init}
    }}
    
    /// Default initializer
    init({init_params}) {{
{state_member_assigns}
    }}
}}
...
{class_attr}
final class {self.class_name}{protocol_decl} {{
