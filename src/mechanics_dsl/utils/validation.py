    ...
    try:
        ...
        if isinstance(value, str):
            try:
                ...
                if not np.isfinite(result):
                    logger.warning(
                        f"safe_float_conversion: non-finite string value '{value}', returning 0.0"
                    )
                    return 0.0
                return result
        ...
        try:
            ...
            if not np.isfinite(result):
                logger.warning(
                    f"safe_float_conversion: non-finite value {type(value).__name__}, returning 0.0"
                )
                return 0.0
            return result
        ...
        logger.error(
            f"safe_float_conversion: unexpected error converting {type(value).__name__}: {e}",
            exc_info=True,
        )
        return 0.0
...
def runtime_type_check(value: Any, expected_type: type, name: str = "value") -> bool:
    ...
    if not isinstance(expected_type, type):
        logger.error(
            f"runtime_type_check: expected_type is not a type: {type(expected_type).__name__}"
        )
        return False
...
def validate_array_safe(
    ...
    if not isinstance(arr, np.ndarray):
        logger.warning(
            f"validate_array_safe: {name} is not numpy.ndarray, got {type(arr).__name__}"
        )
        return False
...
def safe_array_access(arr: np.ndarray, index: int, default: float = 0.0) -> float:
    ...
    try:
        ...
        if not np.isfinite(result):
            logger.warning(
                f"safe_array_access: non-finite value at index {index}, returning default"
            )
            return default
        return result
...
def validate_solution_dict(solution: dict) -> None:
    ...
    if solution["success"]:
        ...
        if y.shape[1] != len(t):
            raise ValueError(
                f"solution['y'] shape mismatch: y.shape[1]={y.shape[1]} != len(t)={len(t)}"
            )
        ...
        if not isinstance(coords, (list, tuple)):
            raise TypeError(
                f"solution['coordinates'] must be list or tuple, got {type(coords).__name__}"
            )
        if len(coords) == 0:
...
def validate_file_path(
    filename: str, must_exist: bool = False, allow_symlinks: bool = False
) -> None:
    """
    ...
    if found_unsafe:
        raise ValueError(f"filename contains unsafe characters: {found_unsafe!r}")
    ...
    with ExitStack() as stack:
        for resource in resources:
            ...
            try:
                ...
                logger.error(
                    f"resource_manager: error adding resource {type(resource).__name__}: {e}"
                )
        yield
