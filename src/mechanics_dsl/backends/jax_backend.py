...
class JAXSolver:
    ...
    def __init__(self, config: Optional[JAXConfig] = None):
        ...
        if not DIFFRAX_AVAILABLE:
            raise ImportError("diffrax is not installed. Install with: pip install diffrax")
    ...
    def solve(
        ...
        **kwargs,
        ...
        stepsize_controller = diffrax.PIDController(rtol=self.config.rtol, atol=self.config.atol)
...
class JAXBackend(Backend):
    ...
    def __init__(self, use_gpu: bool = True, use_64bit: bool = True, jit_compile: bool = True):
        if not JAX_AVAILABLE:
            raise ImportError(
        ...
        self.config = JAXConfig(use_gpu=use_gpu, use_64bit=use_64bit, jit_compile=jit_compile)
    ...
    def compile_equations(
        ...
        for coord in coordinates:
            ...
            func = sp.lambdify([t_sym] + state_symbols, expr, modules=["numpy"])
    ...
    def simulate(
        ...
        **kwargs,
        ...
        return self.solver.solve(equations, t_span, y0, t_eval=t_eval, solver=solver, **kwargs)

    def simulate_batch(
        self, equations: Callable, t_span: Tuple[float, float], y0_batch: np.ndarray, **kwargs
    ) -> np.ndarray:
        """
    ...
    def hessian(self, loss_fn: Callable, params: Dict[str, float]) -> np.ndarray:
        """
