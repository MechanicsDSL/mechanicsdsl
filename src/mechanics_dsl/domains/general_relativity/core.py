...
class GeodesicSolver:
    ...
    def effective_potential(self, r: float, L: float, is_massless: bool = False) -> float:
        """
    ...
    def solve_orbit(
        ...
        def equations(tau, y):
            ...
            if is_massless:
                ...
                d2r_dtau2 = -rs * c**2 * f / (2 * r**2) + L**2 * (r - 1.5 * rs) / r**4
        ...
        return {"tau": sol.t, "r": sol.y[0], "phi": sol.y[1], "t": sol.y[2], "dr_dtau": sol.y[3]}
...
class GravitationalLensing:
    ...
    def __init__(self, mass: float, c: float = SPEED_OF_LIGHT, G: float = GRAVITATIONAL_CONSTANT):
        self.mass = mass
        self.c = c
        self.G = G
    ...
    def einstein_radius(self, D_lens: float, D_source: float) -> float:
        ...
        return np.sqrt(4 * self.G * self.mass * D_ls / (self.c**2 * D_lens * D_source))
    ...
    def time_delay(self, theta_1: float, theta_2: float, D_lens: float, D_source: float) -> float:
        """
        ...
        return (
            ...
            * D_source
            / (self.c * D_ls)
            * (0.5 * (theta_1**2 - theta_2**2) - self.rs / 2 * np.log(abs(theta_1 / theta_2)))
        )
