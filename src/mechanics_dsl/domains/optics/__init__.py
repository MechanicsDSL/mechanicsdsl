...
class RayTracing:
    ...
    @staticmethod
    def ray_at_distance(origin: np.ndarray, direction: np.ndarray, t: float) -> np.ndarray:
...
class ThinLens:
    ...
    @staticmethod
    def lensmaker_equation(n: float, R1: float, R2: float, d: float = 0) -> float:
        """
...
class Interference:
    ...
    @staticmethod
    def thin_film_constructive(n_film: float, d: float, wavelength: float, m: int = 1) -> bool:
        """
...
class Diffraction:
    ...
    @staticmethod
    def grating_equation(d: float, theta_i: float, m: int, wavelength: float) -> Optional[float]:
        """
...
class Polarization:
    ...
    def quarter_wave_plate(theta: float = 0) -> np.ndarray:
        ...
        c, s = math.cos(theta), math.sin(theta)
        return np.array(
            [[c**2 + 1j * s**2, (1 - 1j) * c * s], [(1 - 1j) * c * s, s**2 + 1j * c**2]]
        )
