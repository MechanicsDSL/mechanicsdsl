...
class CanonicalTransformation:
    ...
    def poisson_bracket(self, f: sp.Expr, g: sp.Expr, coordinates: List[str]) -> sp.Expr:
        """
    ...
    def verify_canonical(self, Q: List[sp.Expr], P: List[sp.Expr], old_coords: List[str]) -> bool:
        """
    ...
    def apply_generating_function(
        self, hamiltonian: sp.Expr, gen_func: GeneratingFunction
    ...
    def point_transformation(
        self, hamiltonian: sp.Expr, coord_transform: Dict[str, sp.Expr], coordinates: List[str]
    ) -> CanonicalTransformationResult:
        """
        ...
        for q in coordinates:
            ...
            if q in coord_transform:
                ...
                Q_sym = (
                    list(Q_expr.free_symbols - {q_sym})[0]
                    if Q_expr.free_symbols - {q_sym}
                    else sp.Symbol(f"Q_{q}")
                )
        ...
        return CanonicalTransformationResult(
            new_hamiltonian=sp.simplify(new_H), coordinate_map=coord_map, momentum_map=mom_map
        )

    def exchange_transformation(
        self, hamiltonian: sp.Expr, coordinates: List[str]
    ) -> CanonicalTransformationResult:
        """
...
class HamiltonJacobi:
    ...
    def characteristic_function_1d(
        self, hamiltonian: sp.Expr, coordinate: str, energy: sp.Symbol
    ) -> Optional[sp.Expr]:
        """
    ...
    def solve(
        self, hamiltonian: sp.Expr, coordinates: List[str], energy: Optional[sp.Symbol] = None
    ) -> sp.Expr:
        """
    ...
    def solve_separable(self, hamiltonian: sp.Expr, coordinates: List[str]) -> Dict[str, sp.Expr]:
        """
