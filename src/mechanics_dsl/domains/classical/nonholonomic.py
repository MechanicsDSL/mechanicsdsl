...
class NonholonomicConstraint:
    ...
    def evaluate(self, state: Dict[str, float]) -> float:
        ...
        result = float(self.inhomogeneous.subs({sp.Symbol(k): v for k, v in state.items()}).evalf())

        for coord, coeff in self.coefficients.items():
            vel_name = f"{coord}_dot"
            if vel_name in state:
                coeff_val = float(coeff.subs({sp.Symbol(k): v for k, v in state.items()}).evalf())
...
class NonholonomicSystem:
    ...
    def add_rolling_constraint(
        self, linear_coord: str, angular_coord: str, radius: sp.Expr, direction: str = "forward"
    ) -> sp.Symbol:
        """
        ...
        constraint = NonholonomicConstraint(
            coefficients={linear_coord: sp.S.One, angular_coord: -sign * radius},
            name=f"rolling_{linear_coord}_{angular_coord}",
        )

        return self.add_constraint(constraint)

    def add_knife_edge_constraint(self, x_coord: str, y_coord: str, theta_coord: str) -> sp.Symbol:
        """
        ...
        constraint = NonholonomicConstraint(
            coefficients={x_coord: -sp.sin(theta), y_coord: sp.cos(theta)}, name="knife_edge"
        )
    ...
    def derive_equations_of_motion(self, coordinates: List[str]) -> Dict[str, sp.Expr]:
        ...
        for i, q in enumerate(coordinates):
            ...
            for j, (constraint, lam) in enumerate(zip(self._constraints, self._multipliers)):
                if q in constraint.coefficients:
                    constraint_force += lam * constraint.coefficients[q]

            equations[f"{q}_eq"] = sp.Eq(
                sp.Symbol(f"EL_{q}"), constraint_force  # Placeholder for d/dt(∂L/∂q̇) - ∂L/∂q
            )
    ...
    def get_constraint_matrix(self, coordinates: List[str]) -> Tuple[sp.Matrix, sp.Matrix]:
        """
...
class AppellEquations:
    ...
    def compute_acceleration_energy(
        self, kinetic_energy: sp.Expr, coordinates: List[str]
    ) -> sp.Expr:
        """
    ...
    def derive_appell_equations(
        self,
        acceleration_energy: sp.Expr,
        generalized_forces: Dict[str, sp.Expr],
        pseudo_coordinates: List[str],
    ) -> Dict[str, sp.Expr]:
        """
...
class MaggiEquations:
    ...
    def project_equations(
        self, euler_lagrange: List[sp.Expr], constraint_matrix: sp.Matrix, coordinates: List[str]
    ) -> List[sp.Expr]:
        """
...
def rolling_constraint(linear: str, angular: str, radius: sp.Expr) -> NonholonomicConstraint:
    """Create a rolling without slipping constraint."""
    return NonholonomicConstraint(
        coefficients={linear: sp.S.One, angular: -radius}, name=f"rolling_{linear}"
    )
...
def knife_edge_constraint(x: str, y: str, theta: str) -> NonholonomicConstraint:
    ...
    return NonholonomicConstraint(
        coefficients={x: -sp.sin(theta_sym), y: sp.cos(theta_sym)}, name="knife_edge"
    )
