...
class ScatteringAnalyzer:
    ...
    def compute_scattering_angle(
        ...
        if r_min is None:
            ...
            return ScatteringResult(
                impact_parameter=b, scattering_angle=0.0, closest_approach=float("inf")
            )
        ...
        return ScatteringResult(impact_parameter=b, scattering_angle=theta, closest_approach=r_min)

    def _find_turning_point(
        self, radial_energy: Callable[[float], float], b: float, r_max: float = 1000.0
    ) -> Optional[float]:
    ...
    def coulomb_scattering(
        self, energy: float, impact_parameter: float, k: float, mass: float = 1.0
    ) -> ScatteringResult:
        """
    ...
    def rutherford_differential_cross_section(
        self, k: float, energy: float, theta: float, mass: float = 1.0
    ) -> float:
        """
    ...
    def hard_sphere_scattering(self, radius: float, impact_parameter: float) -> ScatteringResult:
        """
    ...
    def total_cross_section(
        self,
        differential_cs: Callable[[float], float],
...
def rutherford_angle(energy: float, impact_parameter: float, k: float) -> float:
    """
...
def rutherford_cross_section(k: float, energy: float, theta: float) -> float:
    """
