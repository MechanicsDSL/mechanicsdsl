...
class OrbitalElements:
    ...
    def __repr__(self) -> str:
        return (
            f"OrbitalElements(a={self.semi_major_axis:.4f}, e={self.eccentricity:.4f}, "
...
class EffectivePotential:
    ...
    def __init__(
        ...
        mass: float = 1.0,
        r_symbol: Optional[sp.Symbol] = None,
    ):
        """
    ...
    def find_turning_points(
        self, energy: float, r_min_search: float = 0.01, r_max_search: float = 100.0
    ) -> TurningPoints:
        """
        ...
        if len(turning_points) == 0:
            ...
            return TurningPoints(
                r_min=min(turning_points), r_max=max(turning_points), is_bounded=True
            )
...
class CentralForceAnalyzer:
    ...
    def classify_orbit(
        self, potential: sp.Expr, energy: float, angular_momentum: float, mass: float = 1.0
    ) -> OrbitType:
        """
    ...
    def compute_orbital_elements_kepler(
        self, G: float, M: float, m: float, energy: float, angular_momentum: float
    ) -> OrbitalElements:
        """
    ...
    def compute_radial_period(
        ...
        try:
            half_period, _ = quad(
                integrand, turning.r_min * 1.001, turning.r_max * 0.999, limit=100
            )
...
class KeplerProblem:
    ...
    def compute_elements(self, energy: float, angular_momentum: float) -> OrbitalElements:
    ...
    def solve_kepler_equation(
        self, mean_anomaly: float, eccentricity: float, tol: float = 1e-10
    ) -> float:
        """
    ...
    def position_at_time(
        self, t: float, elements: OrbitalElements, t0: float = 0.0
    ) -> Tuple[float, float]:
        """
        ...
        # True anomaly
        phi = 2 * np.arctan2(np.sqrt(1 + e) * np.sin(E / 2), np.sqrt(1 - e) * np.cos(E / 2))
    ...
    def velocity_at_position(
        self, r: float, phi: float, elements: OrbitalElements
    ) -> Tuple[float, float]:
        """
