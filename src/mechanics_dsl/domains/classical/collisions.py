...
class CollisionSolver:
    ...
    def solve(
        ...
        KE_final = 0.5 * m1 * np.dot(v1_final, v1_final) + 0.5 * m2 * np.dot(v2_final, v2_final)
    ...
    def solve_1d(
        self, m1: float, v1: float, m2: float, v2: float, e: float = 1.0
    ) -> Tuple[float, float]:
        """
...
class SymbolicCollisionSolver:
    ...
    def solve_1d_elastic(self) -> Dict[str, sp.Expr]:
        ...
        return {"v1_final": sp.simplify(v1_final), "v2_final": sp.simplify(v2_final)}
...
def inelastic_collision_1d(
    m1: float, v1: float, m2: float, v2: float, e: float
) -> Tuple[float, float]:
    """
...
def perfectly_inelastic_1d(m1: float, v1: float, m2: float, v2: float) -> float:
    """
