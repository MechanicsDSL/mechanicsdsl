...
class NoetherAnalyzer:
    ...
    def detect_cyclic_coordinates(self, lagrangian: sp.Expr, coordinates: List[str]) -> List[str]:
        """
    ...
    def compute_conjugate_momentum(self, lagrangian: sp.Expr, coordinate: str) -> sp.Expr:
        """
    ...
    def compute_energy(self, lagrangian: sp.Expr, coordinates: List[str]) -> sp.Expr:
        """
    ...
    def find_symmetries(self, lagrangian: sp.Expr, coordinates: List[str]) -> List[SymmetryInfo]:
        """
        ...
        if self.is_time_independent(lagrangian):
            ...
            conserved = ConservedQuantity(
                name="energy", expression=energy, symmetry_type=SymmetryType.TIME_TRANSLATION
            )
            symmetries.append(
                SymmetryInfo(
                    symmetry_type=SymmetryType.TIME_TRANSLATION, conserved_quantity=conserved
                )
            )
        ...
        for q in cyclic:
            ...
            symmetries.append(
                SymmetryInfo(
                    symmetry_type=SymmetryType.CYCLIC, coordinate=q, conserved_quantity=conserved
                )
            )
    ...
    def _check_rotation_symmetry(
        self, lagrangian: sp.Expr, coordinates: List[str]
    ) -> Optional[SymmetryInfo]:
        """
        ...
        if rotation_check == 0:
            ...
            return SymmetryInfo(symmetry_type=SymmetryType.ROTATION, conserved_quantity=conserved)

        return None

    def get_all_conserved_quantities(
        self, lagrangian: sp.Expr, coordinates: List[str]
    ) -> Dict[str, sp.Expr]:
        """
    ...
    def verify_conservation(
        self, lagrangian: sp.Expr, quantity: sp.Expr, coordinates: List[str]
    ) -> bool:
        """
...
def detect_cyclic_coordinates(lagrangian: sp.Expr, coordinates: List[str]) -> List[str]:
    """
...
def get_conserved_quantities(lagrangian: sp.Expr, coordinates: List[str]) -> Dict[str, sp.Expr]:
    """
