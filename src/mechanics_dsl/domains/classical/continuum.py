...
class LagrangianDensity:
    ...
    def membrane_lagrangian(self) -> sp.Expr:
        ...
        L = sp.Rational(1, 2) * sigma * u_t**2 - sp.Rational(1, 2) * T * (u_x**2 + u_y**2)
...
class FieldEulerLagrange:
    ...
    def derive_field_equation(
        self, lagrangian: sp.Expr, field: sp.Function, coordinates: List[sp.Symbol]
    ) -> sp.Expr:
        """
...
class VibratingString:
    ...
    def fourier_coefficients(
        self, initial_shape: np.ndarray, x_points: np.ndarray, n_modes: int = 10
    ) -> np.ndarray:
        """
    ...
    def solution(self, x: np.ndarray, t: np.ndarray, coefficients: np.ndarray) -> np.ndarray:
        """
...
class VibratingMembrane:
    ...
    def mode_shape(self, m: int, n: int, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        """
        Get 2D mode shape.
    ...
    def compute_modes(self, max_m: int = 3, max_n: int = 3) -> List[Tuple[int, int, float]]:
        """
...
class StressEnergyTensor:
    ...
    def energy_density(self, lagrangian: sp.Expr, field: sp.Function, time: sp.Symbol) -> sp.Expr:
        """
    ...
    def momentum_density(
        self, lagrangian: sp.Expr, field: sp.Function, time: sp.Symbol, space: sp.Symbol
    ) -> sp.Expr:
        """
...
def string_mode_frequencies(
    length: float, tension: float, density: float, n_modes: int = 5
) -> List[float]:
    """
