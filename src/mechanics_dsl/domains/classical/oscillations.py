...
class NormalModeAnalyzer:
    ...
    def extract_mass_matrix(self, lagrangian: sp.Expr, coordinates: List[str]) -> sp.Matrix:
        """
    ...
    def extract_stiffness_matrix(self, lagrangian: sp.Expr, coordinates: List[str]) -> sp.Matrix:
        """
    ...
    def compute_normal_modes(self, M: np.ndarray, K: np.ndarray) -> List[NormalMode]:
        ...
        for i, (lam, phi) in enumerate(zip(eigenvalues, eigenvectors.T)):
            ...
            if omega_squared < 0:
                logger.warning(
                    f"Mode {i} has negative ω² = {omega_squared}, indicating instability"
                )
                omega = 0.0
                period = float("inf")
    ...
    def modal_decomposition(
        self, M: np.ndarray, K: np.ndarray, F: np.ndarray, omega_drive: float
    ) -> np.ndarray:
        """
...
def compute_normal_modes(
    lagrangian: sp.Expr, coordinates: List[str], parameters: Optional[Dict[str, float]] = None
) -> ModalAnalysisResult:
    """
