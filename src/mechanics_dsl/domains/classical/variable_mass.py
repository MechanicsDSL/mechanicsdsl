...
class RocketEquation:
    ...
    def delta_v_with_gravity(self, params: RocketParameters, g: float = 9.81) -> float:
        """
    ...
    def specific_impulse(self, params: RocketParameters, g0: float = 9.81) -> float:
        """
...
class VariableMassSystem:
    ...
    def derive_eom(
        self, mass_rate: sp.Expr, relative_velocity: sp.Expr, external_force: sp.Expr
    ) -> sp.Expr:
        """
    ...
    def conveyor_belt_force(self, belt_velocity: float, mass_rate: float) -> float:
        """
    ...
    def falling_chain(
        self, chain_length: float, linear_density: float, height: float, g: float = 9.81
    ) -> float:
        """
...
def tsiolkovsky_delta_v(m_initial: float, m_final: float, exhaust_velocity: float) -> float:
    """
    Compute ideal delta-v from Tsiolkovsky equation.
...
def specific_impulse_to_exhaust_velocity(isp: float, g0: float = 9.81) -> float:
    """
