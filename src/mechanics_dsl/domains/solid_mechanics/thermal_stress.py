...
class ThermalStress:
    ...
    def thick_cylinder(
        ...
        sigma_r = factor * (
            -math.log(r_outer / r)
            - r_inner**2 / (r**2 - r_inner**2) * (1 - r_outer**2 / r**2) * math.log(k)
        )
        sigma_theta = factor * (
            1
            - math.log(r_outer / r)
...
class ThermalGradient:
    ...
    @staticmethod
    def linear_gradient_stress(E: float, alpha: float, dT_dy: float, y: float, h: float) -> float:
        """
...
class ThermalShock:
    ...
    @staticmethod
    def first_parameter(sigma_f: float, nu: float, E: float, alpha: float) -> float:
        """First thermal shock parameter R = σf(1-ν)/(Eα)."""
        return sigma_f * (1 - nu) / (E * alpha)

    @staticmethod
    def second_parameter(k: float, sigma_f: float, nu: float, E: float, alpha: float) -> float:
...
class ThermalBuckling:
    ...
    def critical_temperature_plate(
        a: float, b: float, h: float, E: float, nu: float, alpha: float
    ) -> float:
        """
...
class ThermoelasticCoupling:
    ...
    def __init__(self, E: float, alpha: float, rho: float, c_p: float, T_0: float):
        """
...
class HeatConduction:
    ...
    @staticmethod
    def temperature_distribution_plate(T_hot: float, T_cold: float, x: float, L: float) -> float:
...
def compute_thermal_stress(
    E: float, alpha: float, delta_T: float, constrained: bool = True
) -> float:
