...
class IBeamCrossSection(BeamCrossSection):
    ...
    def Ixx(self) -> float:
        ...
        I_flange = 2 * (self.b * self.tf**3 / 12 + 
                        self.b * self.tf * ((self.h - self.tf) / 2)**2)
...
class EulerBernoulliBeam:
    ...
    def natural_frequencies(self, n_modes: int = 3) -> List[float]:
        ...
        if (self.beam.left_support == BeamSupportType.FIXED and 
            self.beam.right_support == BeamSupportType.FREE):
            # Cantilever: βL = 1.875, 4.694, 7.855, ...
            beta_L = [1.875, 4.694, 7.855, 10.996, 14.137]
        elif (self.beam.left_support == BeamSupportType.PINNED and
              self.beam.right_support == BeamSupportType.PINNED):
            # Simply supported: βL = π, 2π, 3π, ...
            beta_L = [n * math.pi for n in range(1, n_modes + 3)]
        elif (self.beam.left_support == BeamSupportType.FIXED and
              self.beam.right_support == BeamSupportType.FIXED):
...
class CantileverBeam(EulerBernoulliBeam):
    ...
    def __init__(self, length: float, section: BeamCrossSection, 
                 material: BeamMaterial = None):
        beam = BeamElement(
            length=length,
...
def compute_beam_deflection(
    ...
    return BeamDeflection(
        ...
        max_deflection_position=x[max_idx]
    )


def compute_bending_stress(
    M: float, section: BeamCrossSection, y: Optional[float] = None
) -> float:
    """
...
def compute_shear_stress_beam(
    V: float, Q: float, I: float, t: float
) -> float:
    """
