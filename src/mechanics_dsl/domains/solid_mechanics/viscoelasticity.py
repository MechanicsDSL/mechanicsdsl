...
class RelaxationSpectrum:
    ...
    def relaxation_modulus(
        self, t: float, tau_min: float = 1e-10, tau_max: float = 1e10, n_points: int = 100
    ) -> float:
        """
...
class MasterCurve:
    ...
    def add_data(self, temp: float, time: np.ndarray, modulus: np.ndarray, shift_factor: float):
...
class TimeTemperatureSuperposition:
    ...
    @staticmethod
    def arrhenius_shift(T: float, T_ref: float, E_a: float, R: float = 8.314) -> float:
        """
...
def compute_relaxation_response(
    model: ViscoelasticMaterial, epsilon_0: float, times: np.ndarray
) -> np.ndarray:
    """
...
def compute_creep_response(
    model: ViscoelasticMaterial, sigma_0: float, times: np.ndarray
) -> np.ndarray:
    """
...
def compute_dynamic_modulus(
    model: GeneralizedMaxwell, frequencies: np.ndarray
...
def fit_prony_series(time: np.ndarray, E_data: np.ndarray, n_terms: int = 5) -> GeneralizedMaxwell:
    """Fit Prony series to relaxation data."""
