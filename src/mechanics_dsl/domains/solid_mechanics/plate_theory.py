...
class PlateElement:
    ...
    def D(self) -> float:
        """Bending rigidity."""
        return BendingRigidity.compute(
            self.material.E, self.thickness, self.material.nu
        )
...
class KirchhoffLovePlate:
    ...
    def rectangular_simply_supported(
        self, a: float, b: float, q: float, 
        m_terms: int = 5, n_terms: int = 5
    ) -> Dict:
        """
    ...
    def rectangular_ss_point_load(
        ...
        for m in range(1, m_terms + 1):
            for n in range(1, n_terms + 1):
                amn = (4 * P / (a * b * D)) * math.sin(m * math.pi * xi / a) * math.sin(n * math.pi * eta / b)
...
class CircularPlate:
    ...
    def clamped_uniform_load(self, q: float) -> Dict:
        ...
        return {
            'max_deflection': w_max,
            'max_radial_moment': Mr_max,
            'center_moment': Mr_center
        }
...
class ThinPlateEquation:
    ...
    def biharmonic_operator(self) -> sp.Expr:
        ...
        ∇⁴w = ∂⁴w/∂x⁴ + 2∂⁴w/∂x²∂y² + ∂⁴w/∂y⁴
        """
        return (sp.diff(self.w, self.x, 4) + 
    ...
    def moment_x(self, nu: sp.Symbol) -> sp.Expr:
        ...
        return -self.D * (sp.diff(self.w, self.x, 2) + 
                         nu * sp.diff(self.w, self.y, 2))
    
    def moment_y(self, nu: sp.Symbol) -> sp.Expr:
        """Bending moment My = -D(∂²w/∂y² + ν∂²w/∂x²)."""
        return -self.D * (sp.diff(self.w, self.y, 2) + 
                         nu * sp.diff(self.w, self.x, 2))
...
class PlateBuckling:
    ...
    @staticmethod
    def uniaxial_compression(a: float, b: float, D: float, 
