...
class StressIntensityFactor:
    ...
    def compact_tension(P: float, a: float, W: float, B: float) -> float:
        ...
        f = (
            (2 + ratio)
            / (1 - ratio) ** 1.5
            * (0.886 + 4.64 * ratio - 13.32 * ratio**2 + 14.72 * ratio**3 - 5.6 * ratio**4)
        )
...
class CTOD:
    ...
    @classmethod
    def from_K(cls, K: float, sigma_y: float, E: float, plane_strain: bool = True) -> "CTOD":
        """
        Compute CTOD from K.
    ...
    @classmethod
    def from_J(cls, J: float, sigma_y: float, plane_strain: bool = True) -> "CTOD":
...
class CrackTipStress:
    ...
    def mode_I(K: float, r: float, theta: float) -> Dict[str, float]:
        ...
        return {"sigma_xx": sigma_xx, "sigma_yy": sigma_yy, "tau_xy": tau_xy}
...
class ParisLaw:
    ...
    def life_to_failure(self, a0: float, af: float, delta_sigma: float, F: float = 1.0) -> int:
        """
...
class MixedModeFracture:
    ...
    @staticmethod
    def failure_envelope_ellipse(KI: float, KII: float, KIc: float, KIIc: float) -> float:
        """
...
def compute_stress_intensity(sigma: float, a: float, geometry: str = "center", **kwargs) -> float:
    """
    ...
    """
    if geometry == "center":
        return StressIntensityFactor.center_crack_tension(sigma, a, kwargs.get("W"))
    elif geometry == "edge":
        return StressIntensityFactor.edge_crack_tension(sigma, a, kwargs["W"])
    elif geometry == "semi_elliptical":
        return StressIntensityFactor.semi_elliptical_surface(sigma, a, kwargs["c"], kwargs["t"])
    else:
...
def compute_plastic_zone(K: float, sigma_y: float, plane_strain: bool = True) -> float:
