...
class ChargedParticle(PhysicsDomain):
    ...
    def derive_equations_of_motion(self) -> Dict[str, sp.Expr]:
        ...
        # Newton's second law
        return {"x_ddot": Fx / self.m, "y_ddot": Fy / self.m, "z_ddot": Fz / self.m}
...
class CyclotronMotion:
    ...
    def exact_trajectory(
        ...
        return {"x": x, "y": y, "z": z, "vx": vx, "vy": vy, "vz": vz, "t": t_array}
...
class GradientDrift:
    ...
    @staticmethod
    def grad_b_drift_velocity(m: float, v_perp: float, q: float, B: float, grad_B: float) -> float:
        """
    ...
    def curvature_drift_velocity(
        m: float, v_parallel: float, q: float, B: float, R_c: float
    ) -> float:
        """
    ...
    @staticmethod
    def polarization_drift_velocity(m: float, q: float, B: float, dE_dt: float) -> float:
        """
...
def plasma_frequency(n_e: float, m_e: float = 9.109e-31, epsilon_0: float = 8.854e-12) -> float:
    """
...
class ElectromagneticWave:
    ...
    def __init__(
        self,
        frequency: float = 1e9,
        amplitude_E: float = 1.0,
        medium_epsilon_r: float = 1.0,
        medium_mu_r: float = 1.0,
    ):
        """
...
class Waveguide:
    ...
    def wave_impedance(self, frequency: float, m: int = 1, n: int = 0, mode: str = "TE") -> float:
        """
