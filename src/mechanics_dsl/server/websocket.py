...
class SimulationStreamer:
    ...
    async def run_simulation(self):
        ...
        def _simulate():
            return self.compiler.simulate(t_span=self.t_span, num_points=self.num_points)
            ...
            while True:
                ...
                if action == "compile":
                    ...
                    if PhysicsCompiler is None:
                        await websocket.send_json(
                            {"type": "error", "message": "PhysicsCompiler not available"}
                        )
                        continue
                    ...
                    if compiler is None:
                        await websocket.send_json({"type": "error", "message": "No code compiled"})
                        continue
                    ...
                    if streamer:
                        ...
                        streamer.seek(frame)
                        await websocket.send_json({"type": "seeked", "frame": frame})
