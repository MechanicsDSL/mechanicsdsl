...
if PYDANTIC_AVAILABLE:
    ...
    class ServerConfig(BaseModel):
        ...
        # nosec B104: 0.0.0.0 bind is intentional for Docker container deployment
...
def validate_simulation_config(config: Dict[str, Any]) -> SimulationConfig:
    ...
    if PYDANTIC_AVAILABLE:
        ...
        return SimulationConfig(
            **{k: v for k, v in config.items() if k in SimulationConfig.__dataclass_fields__}
        )


def validate_coordinate(
    name: str, value: float, velocity: float = 0.0, bounds: Optional[Tuple[float, float]] = None
) -> CoordinateConfig:
    """Validate coordinate configuration."""
    if PYDANTIC_AVAILABLE:
        return CoordinateConfig(
            name=name, initial_value=value, initial_velocity=velocity, bounds=bounds
        )
    else:
        return CoordinateConfig(
            name=name, initial_value=value, initial_velocity=velocity, bounds=bounds
...
def validate_parameter(
    name: str, value: float, min_value: Optional[float] = None, max_value: Optional[float] = None
) -> ParameterConfig:
    """Validate parameter configuration."""
    if PYDANTIC_AVAILABLE:
        return ParameterConfig(name=name, value=value, min_value=min_value, max_value=max_value)
    else:
        # Manual validation
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if PYDANTIC_AVAILABLE and isinstance(e, PydanticValidationError):
                raise ValidationError(str(e), errors=[err for err in e.errors()])
            raise
