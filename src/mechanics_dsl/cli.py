...
def cmd_run(args):
    ...
    if HAS_TQDM:
        with tqdm(
            total=3, desc="MechanicsDSL", bar_format="{desc}: {bar} {percentage:3.0f}%"
        ) as pbar:
            ...
            if not result.get("success", False):
                print(
                    f"Compilation failed: {result.get('error', 'Unknown error')}", file=sys.stderr
                )
                return 1
    ...
    if args.output:
        ...
        output_data = {
            "t": (
                solution["t"].tolist() if hasattr(solution["t"], "tolist") else list(solution["t"])
            ),
...
Examples:
  ...
  mechanicsdsl info
    ...
    compile_parser.add_argument(
        "--target", "-t", required=True, help="Target language (cpp, cuda, rust, julia, etc.)"
    )
    ...
    run_parser.add_argument(
        "--points", "-n", type=int, default=1000, help="Number of time points (default: 1000)"
    )
    ...
    export_parser.add_argument(
        "--format", "-f", required=True, help="Output format (json, csv, numpy)"
    )
