...
\authortodo{To write. Differential testing; metamorphic testing of scientific
software; verification and validation of multibody codes; prior engine
comparisons. No citations are supplied here: inventing them would be worse than
leaving the section empty, and the surrounding argument does not depend on any
particular prior result.}
...
An AI assistant (Anthropic Claude) was used throughout this work: in deriving
and implementing the reference and the engine adapters, in constructing and
executing the sweeps, in analysing the resulting records, and in drafting this
manuscript. Every numerical result reported was produced by executing committed
code, and the author verified each figure against the run record. The assistant
also produced several of the erroneous expectations recorded in
\S\ref{sec:threats} and \S\ref{sec:diagnostic}; those were detected by the same
artefact-checking procedure described there, and are reported rather than
removed. This disclosure is made because concealing such use is not permissible,
and because the errors it produced are part of the evidence that the process
detects its own faults.
...
and the raw output records --- are available at \authortodo{Zenodo DOI: deposit
the artefacts and insert the resolved DOI here before posting. This is the one
blank that cannot be filled from the run record.}
...
\paragraph{Use of AI assistance.}
An AI assistant (Anthropic Claude) was used throughout this work: in deriving
and implementing the library-independent reference and the three engine
adapters, in constructing and executing the sweeps, in analysing the resulting
records, and in drafting this manuscript. Every numerical result reported was
produced by executing committed code, and the author verified each figure
against the run record. The assistant also produced several of the erroneous
expectations recorded in \S\ref{sec:threats} and \S\ref{sec:diagnostic}; those
were detected by the artefact-checking procedure described there and are
reported rather than removed, because they are part of the evidence that the
process detects its own faults.
...
\authortodo{Zenodo DOI --- deposit before posting so the link resolves}.
