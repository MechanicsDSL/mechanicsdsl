...
class TestEnergyConservation:
    ...
    def test_double_pendulum_energy(self):
        ...
        T = (
            sp.Rational(1, 2) * (m1_s + m2_s) * L1_s**2 * theta1_dot**2
            + sp.Rational(1, 2) * m2_s * L2_s**2 * theta2_dot**2
            + m2_s * L1_s * L2_s * theta1_dot * theta2_dot * sp.cos(theta1 - theta2)
        )

        V = -(m1_s + m2_s) * g_s * L1_s * sp.cos(theta1) - m2_s * g_s * L2_s * sp.cos(theta2)
        ...
        sim.set_initial_conditions(
            {"theta1": theta1_0, "theta1_dot": 0.0, "theta2": theta2_0, "theta2_dot": 0.0}
        )
        ...
        V_val = -(m1 + m2) * g * L1 * np.cos(th1) - m2 * g * L2 * np.cos(th2)
        E = T_val + V_val
    ...
    def test_coupled_oscillators_energy(self):
        ...
        T = (
            sp.Rational(1, 2) * engine.get_symbol("m1") * v1**2
            + sp.Rational(1, 2) * engine.get_symbol("m2") * v2**2
        )
        V = (
            sp.Rational(1, 2) * engine.get_symbol("k1") * x1**2
...
class TestMomentumConservation:
    ...
    def test_two_body_momentum_conservation(self):
        ...
        sim.set_initial_conditions({"x1": 0.0, "x1_dot": 1.0, "x2": 2.0, "x2_dot": -0.5})
        sim.compile_equations(accels, ["x1", "x2"])
    ...
    def test_central_force_angular_momentum(self):
        ...
        lagrangian = (
            sp.Rational(1, 2) * m * (r_dot**2 + r**2 * phi_dot**2) - sp.Rational(1, 2) * k * r**2
        )
        ...
        sim.set_initial_conditions({"r": 1.0, "r_dot": 0.0, "phi": 0.0, "phi_dot": 2.0})
...
class TestNoetherSymmetries:
    ...
    def test_space_translation_momentum(self):
        ...
        assert (
            total_force_simplified == 0
        ), f"Total external force should be zero: {total_force_simplified}"
    ...
    def test_rotation_angular_momentum(self):
        ...
        assert (
            sp.simplify(p_theta_simplified - expected) == 0
