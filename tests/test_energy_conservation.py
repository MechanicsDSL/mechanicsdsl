...
class TestEnergyConservation:
    ...
    def test_harmonic_oscillator_energy(self):
        ...
        if E_total[0] != 0:
            ...
            assert max_error < 0.01, f"Energy conservation violated: max error {max_error:.6f}"
            
            # Mean error should be even smaller
            mean_error = np.mean(energy_error)
            assert mean_error < 0.005, f"Mean energy error too large: {mean_error:.6f}"
    ...
    def test_pendulum_energy(self):
        ...
        if E_total[0] != 0:
            energy_error = np.abs((E_total - E_total[0]) / E_total[0])
            assert np.max(energy_error) < 0.05, f"Energy error: {np.max(energy_error):.6f}"
    ...
    def test_coupled_oscillators_energy(self):
        ...
        if E_total[0] != 0:
            energy_error = np.abs((E_total - E_total[0]) / E_total[0])
            assert np.max(energy_error) < 0.05
