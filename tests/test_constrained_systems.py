...
class TestRollingBall:
    ...
    def test_rolling_ball_constraint(self):
        ...
        x = solution['y'][0]
        assert x[-1] > x[0]  # Should move forward
        assert np.all(x >= 0)  # Should not go backwards
...
class TestAtwoodMachine:
    ...
    def test_atwood_machine(self):
        ...
        if len(solution['coordinates']) > 0:
            assert x1[-1] < x1[0] or x1[-1] > x1[0]  # Should move
...
class TestPendulumWithConstraint:
    ...
    def test_constrained_pendulum(self):
        ...
        assert np.max(constraint_error) < 0.1, f"Constraint violated: max error {np.max(constraint_error)}"
