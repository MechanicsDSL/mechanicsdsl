...
class TestEquilibriumFinding:
    ...
    def test_pendulum_equilibria(self):
        ...
        equilibria = find_equilibria(V, ["theta"], bounds={"theta": (-4, 4)})

        # Should find at least θ = 0 (stable) and θ = π (unstable)
        assert len(equilibria) >= 1

        # Check θ = 0 is found and is minimum
        theta_zero = [eq for eq in equilibria if abs(eq.coordinates["theta"]) < 0.1]
...
class TestLinearization:
    ...
    def test_coupled_oscillators_matrices(self):
        ...
        V = (
            sp.Rational(1, 2) * k * x1**2
            + sp.Rational(1, 2) * k * x2**2
...
class TestNormalModes:
    ...
    def test_two_dof_modes(self):
        ...
        np.testing.assert_array_almost_equal(np.sort(frequencies), [np.sqrt(2), 2.0], decimal=5)
