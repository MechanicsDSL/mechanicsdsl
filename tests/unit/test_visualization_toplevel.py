...
def minimal_solution():
    ...
    return {"success": True, "t": t, "y": y, "coordinates": ["q"]}
...
class TestSaveAnimationToFile:
    ...
    def test_save_animation_invalid_fps_type(self, visualizer, temp_dir):
        ...
        with pytest.raises(TypeError, match="fps must be int"):
            visualizer.save_animation_to_file(
                mock_anim, os.path.join(temp_dir, "test.gif"), fps="30"
            )
    ...
    def test_save_animation_fps_too_high(self, visualizer, temp_dir):
        ...
        with pytest.raises(ValueError, match="fps must be in"):
            visualizer.save_animation_to_file(
                mock_anim, os.path.join(temp_dir, "test.gif"), fps=200
            )
    ...
    def test_save_animation_invalid_dpi_type(self, visualizer, temp_dir):
        ...
        with pytest.raises(TypeError, match="dpi must be int"):
            visualizer.save_animation_to_file(
                mock_anim, os.path.join(temp_dir, "test.gif"), dpi="100"
            )
    ...
    def test_save_animation_dpi_too_high(self, visualizer, temp_dir):
        ...
        with pytest.raises(ValueError, match="dpi must be in"):
            visualizer.save_animation_to_file(
                mock_anim, os.path.join(temp_dir, "test.gif"), dpi=2000
            )
    ...
    @patch.object(MechanicsVisualizer, "has_ffmpeg", return_value=False)
    def test_save_animation_other_format_without_ffmpeg_fails(
        self, mock_ffmpeg, visualizer, temp_dir
    ):
...
class TestAnimatePendulum:
    ...
    def test_animate_pendulum_invalid_system_name_type(
        self, visualizer, single_pendulum_solution, pendulum_parameters
    ):
    ...
    def test_animate_pendulum_failed_solution(
        self, visualizer, failed_solution, pendulum_parameters
    ):
    ...
    def test_animate_single_pendulum(
        self, visualizer, single_pendulum_solution, pendulum_parameters
    ):
        """Test animating single pendulum."""
        anim = visualizer.animate_pendulum(
            single_pendulum_solution, pendulum_parameters, "pendulum"
        )
        assert anim is not None
        assert isinstance(anim, animation.FuncAnimation)

    def test_animate_double_pendulum(
        self, visualizer, double_pendulum_solution, double_pendulum_parameters
    ):
        """Test animating double pendulum."""
        anim = visualizer.animate_pendulum(
            double_pendulum_solution, double_pendulum_parameters, "double_pendulum"
        )
        assert anim is not None
        assert isinstance(anim, animation.FuncAnimation)

    def test_animate_double_pendulum_with_multi_coords(
        self, visualizer, double_pendulum_solution, double_pendulum_parameters
    ):
        """Test animating with 2+ coordinates triggers double pendulum."""
        anim = visualizer.animate_pendulum(
            double_pendulum_solution, double_pendulum_parameters, "system"
        )
        assert anim is not None
...
class TestAnimateSinglePendulumPrivate:
    ...
    def test_single_pendulum_valid_state(
        self, visualizer, single_pendulum_solution, pendulum_parameters
    ):
        """Test single pendulum with valid state."""
        visualizer.setup_3d_plot()
        anim = visualizer._animate_single_pendulum(
            single_pendulum_solution["t"], single_pendulum_solution["y"], pendulum_parameters
        )
        assert anim is not None
    ...
    def test_single_pendulum_default_length(self, visualizer, single_pendulum_solution):
        ...
        anim = visualizer._animate_single_pendulum(
            single_pendulum_solution["t"], single_pendulum_solution["y"], params
        )
        assert anim is not None
...
class TestAnimateDoublePendulumPrivate:
    ...
    def test_double_pendulum_valid_state(
        self, visualizer, double_pendulum_solution, double_pendulum_parameters
    ):
        """Test double pendulum with valid state."""
        visualizer.setup_3d_plot()
        anim = visualizer._animate_double_pendulum(
            double_pendulum_solution["t"], double_pendulum_solution["y"], double_pendulum_parameters
        )
        assert anim is not None

    def test_double_pendulum_uses_single_length_fallback(
        self, visualizer, double_pendulum_solution
    ):
        """Test double pendulum uses 'l' if 'l1' not provided."""
        visualizer.setup_3d_plot()
        params = {"l": 1.5}  # Only 'l', not 'l1'
        anim = visualizer._animate_double_pendulum(
            double_pendulum_solution["t"], double_pendulum_solution["y"], params
        )
        assert anim is not None
...
class TestAnimateOscillator:
    ...
    def test_animate_oscillator_failed_solution(
        self, visualizer, failed_solution, pendulum_parameters
    ):
    ...
    def test_animate_oscillator_valid_solution(
        self, visualizer, oscillator_solution, pendulum_parameters
    ):
        """Test animating oscillator."""
        anim = visualizer.animate_oscillator(
            oscillator_solution, pendulum_parameters, "harmonic_oscillator"
        )
        assert anim is not None
        assert isinstance(anim, animation.FuncAnimation)

    def test_animate_oscillator_empty_state(
        self, visualizer, empty_state_solution, pendulum_parameters
    ):
        """Test oscillator with empty state."""
        result = visualizer.animate_oscillator(empty_state_solution, pendulum_parameters)
        assert result is None

    def test_animate_oscillator_minimal_state(
        self, visualizer, minimal_solution, pendulum_parameters
    ):
        """Test oscillator with minimal state (only position)."""
        anim = visualizer.animate_oscillator(minimal_solution, pendulum_parameters)
        assert anim is not None

    def test_animate_oscillator_sets_fig_and_ax(
        self, visualizer, oscillator_solution, pendulum_parameters
    ):
...
class TestAnimateDispatcher:
    ...
    def test_animate_pendulum_by_coords(
        self, visualizer, single_pendulum_solution, pendulum_parameters
    ):
    ...
    def test_animate_oscillator_by_spring_name(
        self, visualizer, oscillator_solution, pendulum_parameters
    ):
    ...
    def test_animate_oscillator_by_single_x_coord(
        self, visualizer, oscillator_solution, pendulum_parameters
    ):
...
class TestPlotEnergy:
    ...
    def test_plot_energy_valid_solution(
        self, mock_show, visualizer, single_pendulum_solution, pendulum_parameters
    ):
    ...
    @patch("matplotlib.pyplot.show")
    def test_plot_energy_with_lagrangian(
        self, mock_show, visualizer, single_pendulum_solution, pendulum_parameters
    ):
        ...
        visualizer.plot_energy(
            single_pendulum_solution, pendulum_parameters, "pendulum", lagrangian=L
        )
        mock_show.assert_called_once()
...
class TestPlotPhaseSpace:
    ...
    def test_plot_phase_space_invalid_coordinate_index_type(
        self, visualizer, single_pendulum_solution
    ):
    ...
    def test_plot_phase_space_coordinate_index_out_of_range(
        self, visualizer, single_pendulum_solution
    ):
    ...
    def test_plot_phase_space_with_nan_values(self, visualizer):
        ...
        t = np.linspace(0, 5, 50)
        y = np.vstack([np.where(t > 2, np.nan, np.sin(t)), np.where(t > 2, np.nan, np.cos(t))])
        solution = {"success": True, "t": t, "y": y, "coordinates": ["theta"]}
    ...
    def test_plot_phase_space_with_inf_values(self, visualizer):
        ...
        t = np.linspace(0, 5, 50)
        y = np.vstack([np.where(t > 2, np.inf, np.sin(t)), np.where(t > 2, -np.inf, np.cos(t))])
        solution = {"success": True, "t": t, "y": y, "coordinates": ["theta"]}
        # Validation catches non-finite values
        with pytest.raises(ValueError, match="non-finite"):
            visualizer.plot_phase_space(solution, 0)
...
class TestAnimationFrameFunctions:
    ...
    def test_single_pendulum_animate_frame(
        self, visualizer, single_pendulum_solution, pendulum_parameters
    ):
    ...
    def test_double_pendulum_animate_frame(
        self, visualizer, double_pendulum_solution, double_pendulum_parameters
    ):
...
class TestEdgeCases:
    ...
    def test_animate_with_none_system_name(
        self, visualizer, single_pendulum_solution, pendulum_parameters
    ):
    ...
    def test_animate_pendulum_with_empty_system_name(
        self, visualizer, single_pendulum_solution, pendulum_parameters
    ):
    ...
    def test_animate_with_double_in_name(
        self, visualizer, double_pendulum_solution, double_pendulum_parameters
    ):
        """Test that 'double' in name triggers double pendulum."""
        anim = visualizer.animate_pendulum(
            double_pendulum_solution, double_pendulum_parameters, "my_double_system"
        )
        assert anim is not None
